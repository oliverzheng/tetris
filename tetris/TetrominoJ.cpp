...
int TetrominoJ::setup_positions_x[] = {Tetris::getWidth() / 2 - 1,
									   Tetris::getWidth() / 2,
									   Tetris::getWidth() / 2 + 1,
									   Tetris::getWidth() / 2 + 1};
int TetrominoJ::setup_positions_y[] = {2, 2, 2, 3};

// Relative positions that should be displayed in the Tetris NEXT box
int TetrominoJ::next_positions_x[] = {0, 1, 2, 2};
int TetrominoJ::next_positions_y[] = {0, 0, 0, 1};

// Relative positions that each block should move to
int TetrominoJ::rotation_positions_x[][block_count] = {{1, 0, -1, -2},
													   {1, 0, -1, 0},
													   {-1, 0, 1, 2},
													   {-1, 0, 1, 0}};
int TetrominoJ::rotation_positions_y[][block_count] = {{-1, 0, 1, 0},
													   {1, 0, -1, -2},
													   {1, 0, -1, 0},
													   {-1, 0, 1, 2}};
...
Output:
	None
******************************************************************************/
TetrominoJ::TetrominoJ() : rotation_state(0) {
...
Output:
	none
******************************************************************************/
void TetrominoJ::nextSetup(void) {
	for(int i = 0; i < block_count; i++) {
